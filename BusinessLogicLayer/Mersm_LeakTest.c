...
static void Mersm_evDelayOver(Mersm_Leak *const me)
{
  ...
  case SUB_MEAS_4:
    ...
    if(private_data->m_stateID == SUB_MEAS_4){
      //干燥后，小于压差才继续测试
      float tempPres = me->m_pAD->getPresData(me->m_pAD, TEST_PRES_DATA);
      if (500 < fabsf(tempPres - private_data->m_measPresRef))
      {
        //不满足测试条件
        testCondition = 1;
     }
...
void enterSUB_AIR_PUMP(Mersm_Leak *const me)
{
  ...
  for (i = WATER_IN_V; i < MAX_NUM_V; i++)
  {
        ...
        (i == PRES_TO_WATER_V) || (i == CYL_H_V))
    {
      ...
      me->m_pValve->setVal(me->m_pValve, i, 0);
  }
  ...
  if (LEAK_LOW_PRES == private_data->m_type)
    msg.pVoid = (void *)MB_STATE_L_PUMP;
  else
    msg.pVoid = (void *)MB_STATE_H_PUMP;
  msg.dataType = MB_UPDATE_STATE;
  xQueueSend(g_modbusQ, &msg, 10);

  /*根据当前状态是低压平衡，还是高压平衡,来确定平衡延时*/
  if (LEAK_LOW_PRES == private_data->m_type)
  {
    //开启低压抽气倒计时定时器
    me->m_pDelay->setDelayTime(me->m_pDelay, me->m_pTimePara->LowAirPump);
  }
  else
  {
    //开启高压抽气倒计时定时器
    me->m_pDelay->setDelayTime(me->m_pDelay, me->m_pTimePara->HighAirPump);
  }
...
void enterSUB_BALANCE(Mersm_Leak *const me)
{
  ...
  AppMesage msg;
  LeakTestData *private_data = (LeakTestData *)me->private_data;
  float tempF = me->m_pAD->getPresData(me->m_pAD, CUR_WATER_DATA); //获取水压
  //增加低压测试条件下， 水压应该满足低压测试条件
  if ((LEAK_LOW_PRES == private_data->m_type) && ((tempF < me->m_pPresPara->lowWaterMin) || (tempF > me->m_pPresPara->lowWaterMax))){
      //水压故障
      testCondition = 2;    
      private_data->m_stateID = SUB_NULL;
      ActionEnterArr[SUB_NULL](me); 
    ...
    /*给MODBUS发消息更新状态*/
    if (LEAK_LOW_PRES == private_data->m_type)
      msg.pVoid = (void *)MB_STATE_L_BAL;
    else
      msg.pVoid = (void *)MB_STATE_H_BAL;
    msg.dataType = MB_UPDATE_STATE;
    xQueueSend(g_modbusQ, &msg, 10);
    ...
    if (LEAK_LOW_PRES == private_data->m_type)
    {
      me->m_pDelay->setDelayTime(me->m_pDelay, me->m_pTimePara->LowBalance);
    }
    else
    {
      me->m_pDelay->setDelayTime(me->m_pDelay, me->m_pTimePara->HighBalance);
    }
  }
...
void enterSUB_MEAS_1(Mersm_Leak *const me)
{
  ...
  if (PUMP_BAL_DIFF < fabsf(tempPres - private_data->m_measPresRef))
  {
    ...
    //只有在满足干燥和平衡压力下，才继续进行检测，小于压差才继续测试
    for (i = WATER_IN_V; i < MAX_NUM_V; i++)
    {
...
void enterSUB_DRY_1(Mersm_Leak *const me)
{
  ...
  /*给MODBUS发消息更新状态*/
  if (LEAK_LOW_PRES == private_data->m_type)
    msg.pVoid = (void *)MB_STATE_L_1_DRY;
  else
    msg.pVoid = (void *)MB_STATE_H_1_DRY;
  msg.dataType = MB_UPDATE_STATE;
  xQueueSend(g_modbusQ, &msg, 10);
...
void enterSUB_MEAS_2(Mersm_Leak *const me)
{
  ...
  //增加干燥后，小于压差才继续测试
  float tempPres = me->m_pAD->getPresData(me->m_pAD, TEST_PRES_DATA);
  if (300 < fabsf(tempPres - private_data->m_measPresRef))
  {
    //不满足测试条件
    testCondition = 1;
    //pErr->setErrBit(ERR_NEED_DRY);
...
void enterSUB_DRY_2(Mersm_Leak *const me)
{
  ...
  /*给MODBUS发消息更新状态*/
  if (LEAK_LOW_PRES == private_data->m_type)
    msg.pVoid = (void *)MB_STATE_L_2_DRY;
  else
    msg.pVoid = (void *)MB_STATE_H_2_DRY;
  msg.dataType = MB_UPDATE_STATE;
  xQueueSend(g_modbusQ, &msg, 10);
...
void enterSUB_DRY_3(Mersm_Leak *const me)
{
  ...
  /*给MODBUS发消息更新状态*/
  if (LEAK_LOW_PRES == private_data->m_type)
    msg.pVoid = (void *)MB_STATE_L_3_DRY;
  else
    msg.pVoid = (void *)MB_STATE_H_3_DRY;
  msg.dataType = MB_UPDATE_STATE;
  xQueueSend(g_modbusQ, &msg, 10);
...
static void leakDispCb(void *pInstance, void *pData)
{
  ...
  if ((s_count % 10) == 0)
  {
    if (LEAK_LOW_PRES == private_data->m_type)
    {
      if (SUB_MEAS_1 == private_data->m_stateID)
        msg.dataType = MB_UPDATE_LOW_1;
      else if (SUB_MEAS_2 == private_data->m_stateID)
        msg.dataType = MB_UPDATE_LOW_2;
      else if (SUB_MEAS_3 == private_data->m_stateID)
        msg.dataType = MB_UPDATE_LOW_3;
      else if (SUB_MEAS_4 == private_data->m_stateID)
    ...
    msg.pVoid =(void *) convFloat2u32(leakRate);
...
static void caclResAndSend(Mersm_Leak *const me)
{
  ...
  if (LEAK_LOW_PRES == private_data->m_type)
  {
    ...
    {
    case SUB_MEAS_1:
      if (leakRate > me->m_pPresPara->lowLeakRate)
    ...
    msg.pVoid =(void *) convFloat2u32(leakRate); //浮点数转化为u32格式
    xQueueSend(g_modbusQ, &msg, 10);
  }
  else
  {
...
static void addNgCount(u8 port)
{
  ...
  if (1 == port)
    ...
    pErr->setVbRes(EX_NG2_S);
  else if (3 == port)
    pErr->setVbRes(EX_NG3_S);
  else if (4 == port)
    pErr->setVbRes(EX_NG4_S);
}
...
{
  FlagHand *pErr = errHandCreate(); //错误处理对象
  s_ngCount &= ~(0x03 << (2 * (port - 1)));
  if (1 == port)
    pErr->setVbRes(EX_OK1_S);
  else if (2 == port)
    pErr->setVbRes(EX_OK2_S);
  else if (3 == port)
    pErr->setVbRes(EX_OK3_S);
  else if (4 == port)
    pErr->setVbRes(EX_OK4_S);
}
