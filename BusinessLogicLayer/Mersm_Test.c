 ...
 * @LastEditors: PanHan
 * @LastEditTime: 2020-05-04 16:11:47
...
static void Mersm_evStop(void)
{
  ...
  /*收到停止消息后，如果干燥信号属于触发状态，则需要清除错误信号，便于机械手启动干燥*/
  if (EX_DRY_S & pErr->getAllVbStatus()){
...
static void Mersm_evDelayOver(void)
{
  ...
  case STATE_LOW_PRES_DETEC:
    ...
    //只有子流程处于停止状态时，才会根据真空传感器状态判断后续走向,增加在测试条件不满足的情况下，直接排水
    if (SUB_NULL == me->its_pSubLowPresLeak->getCurrentState(me->its_pSubLowPresLeak)){
      if ((1 == me->m_pExitSig->getExitSing(me->m_pExitSig, VACUUM_SING)) || \
          (0x0f == u32Temp) || (0 != testCondition)){
        if(testCondition == 2){
            pErr->setErrBit(ERR_WATER_PRES);
            private_data->m_stateID = STATE_WATER_EXH;
            enterWATER_EXH(me);
        }else{
          // /*提前停止，则根据低压结果来更新NG计数,在低压测试条件不满足的情况下，不更新*/
          /*
            if(testCondition != 1){
              for (i = 0; i < 4; i ++){
           ...
           */
          me->its_pSubLowPresLeak->clearAllNgCount();
          private_data->m_stateID = STATE_WATER_EXH;
          enterWATER_EXH(me);
  ...
  case STATE_ADD_PRES:
    ...
    /*加压结束需要检测水压是否在设定范围内*/
    if (me->m_pExitSig->getExitSing(me->m_pExitSig, WATER_DECT_SING) && (tempF > me->m_pPresPara->highWaterMin) && (tempF < me->m_pPresPara->highWaterMax)){
